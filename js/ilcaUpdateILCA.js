...
export function trackRaceLegs(map) {
  ...
  // Maps calculations directly to your exact UI dashboard property keys at every single tick!
  ...
  // Save them inside the ILCA sub-object so your AI history recorder function grabs them perfectly!
  ...
  // Save a structural state snapshot into the history array block on every single second tick
  ...
  if (distanceToTarget <= roundingRadius) {
    ...
    if (nextMarkLat !== undefined && nextMarkLon !== undefined && window.globalSimulationData.ILCA) {
      ...
      if (typeof showNotification === "function") showNotification(`Auto-Pilot: Course adjusted to ${newHeading.toFixed(0)}°.`);
    }
  } else {
