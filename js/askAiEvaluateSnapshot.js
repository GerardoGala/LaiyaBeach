...
export function evaluateSnapshot(sim, container) {
  ...
  try {
    container.innerHTML = ""; // Clear the "Calculating metrics..." placeholder instantly
    ...
    if (ilca.pointOfSail === "In Irons" || (Number(sim.speed) === 0 && ilca.pointOfSail === "Close Hauled" && Math.abs(((Number(ilca.heading) - Number(sim.windDirection) + 540) % 360) - 180) < 10)) {
      container.innerHTML = `
        <div class='tips-list'>
          <div class='tip-item critical-error' style='border-left-color: #dc3545; background: #fff5f5; color: #721c24; padding: 12px; font-size: 14px;'>
            ⛵ <strong>You are In Irons!</strong> You are pointing directly into the wind with zero speed. 
            <br><br>
            <strong>How to fix it:</strong> Push your tiller hard forward, let your sail out completely flat, and allow the wind to drift your bow sideways onto a <strong>Close Hauled</strong> heading before trimming your lines!
          </div>
        </div>`;
      return;
    }
    ...
    console.error("Ask AI runtime diagnostics crash caught: ", err);
    container.innerHTML = "<div class='tip-item critical-error'>❌ Internal System Error: Failed compiling active performance metrics cards.</div>";
  }
}
